...
<div>
    <a class="btn btn-danger btn-sm" href="{{ route('admin.home-testimonials.create') }}">
        Add
    </a>
</div>
        ...
        <tbody>
            @foreach ($contents as $index => $content)
                <tr>
                    ...
                    <td>
                        <a href="{{ route('admin.home-testimonials.edit', $content->id) }}">
                            <i class="fas fa-edit"></i>
                        </a>
                        <a class="deleteRecord" data-id="form-submit-{{ $content->id }}"
                            data-route="{{ route('admin.home-testimonials.destroy', $content->id) }}">
                            <i class="fas fa-trash" style="color:rgb(255, 23, 23)"></i>
                        </a>
                        <form method="POST" id="form-submit-{{ $content->id }}"
                            action="{{ route('admin.home-testimonials.destroy', $content->id) }}" hidden>
                            @method('DELETE')
                            @csrf
